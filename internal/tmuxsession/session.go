...
func Launch(cfg config.Paths) error {
	...
	commands := [][]string{
		...
		{"set-option", "-t", session, "-g", "status-left", ""},
		{"set-option", "-t", session, "-g", "status-right", ""},
		{"set-option", "-t", session, "-g", "status-left-length", "0"},
		{"set-option", "-t", session, "-g", "status-right-length", "0"},
		{"set-option", "-t", session, "-g", "status-justify", "centre"},
		{"set-option", "-t", session, "-g", "window-status-separator", "  "},
		{"set-option", "-t", session, "-g", "window-status-format", " Editor #[dim]|#[default] Terminal #[dim]|#[default] Git #[dim]|#[default] Melo "},
		{"set-option", "-t", session, "-g", "window-status-current-format", "#[bold] #W #[default]"},
		{"set-option", "-t", session, "-g", "allow-rename", "off"},
		{"bind-key", "-n", "C-c", "kill-session", "-t", session},
		...
		{"new-window", "-t", session, "-n", "Melo", nvimCmd},
		{"set-window-option", "-t", session + ":Melo", "pane-border-status", "top"},
		{"split-window", "-t", session + ":Melo.0", "-v", "-p", "25", lazygitCmd},
		{"select-pane", "-t", session + ":Melo.0"},
		{"split-window", "-t", session + ":Melo.0", "-h", "-p", "40", docsCmd},
		{"select-pane", "-t", session + ":Melo.0"},
